...
export async function contrastOf(page: Page, route: string): Promise<PageContrast> {
  ...
  return { path: route, violatingElements: nodes.length, nodes };
}
...
export function writeContrastReport(app: string, theme: string, pages: PageContrast[]): void {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ app, theme, envelope: ENVELOPE, totalViolatingElements: total, pages }, null, 2),
  );
  const rows = pages
  ...
  console.log(
    `\n[contrast] ${app} ${theme}: ${total} failing elements on ${pages.filter((p) => p.violatingElements).length}/${pages.length} pages\n${rows.join("\n")}\n`,
  );
}
