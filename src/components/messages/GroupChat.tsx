...
interface Message {
  ...
  duration?: string;
  _updated?: number; // Timestamp to force re-render when message ownership is corrected
}
...
export default function GroupChat({
  ...
  const [messagesNeedRevaluation, setMessagesNeedRevaluation] =
  ...
  }, [user, messages]); // Include messages to detect when we have messages that need re-evaluation

  // Re-evaluate message senderType when user authentication loads OR when messages need re-evaluation
  useEffect(() => {
    if (!user || messages.length === 0) return;

    // Check if any messages have incorrect senderType (are marked as "other" but should be "self")
    const needsCorrection = messages.some((msg) => {
      const isMyMessage = isCurrentUser(msg.senderId || "", msg.sender);
      const shouldBeSelf = isMyMessage;
      const currentlyOther = msg.senderType === "other";
      return shouldBeSelf && currentlyOther;
    });

    if (!needsCorrection && !messagesNeedRevaluation) return;

    setMessages((prevMessages) => {
      const updatedMessages = prevMessages.map((msg) => {
        const isMyMessage = isCurrentUser(msg.senderId || "", msg.sender);
        const newSenderType = isMyMessage
          ? "self"
          : getUserRole(msg.senderId || "");

        if (msg.senderType !== newSenderType) {
          // Create a new object to force re-render
          return {
            ...msg,
            senderType: newSenderType,
            // Add a timestamp to force component re-render
            _updated: Date.now(),
          };
        }
      ...
      const hasUpdates = updatedMessages.some(
        (msg, index) => msg.senderType !== prevMessages[index]?.senderType
      );
      ...
      // Reset the flag even if no updates were needed
      setMessagesNeedRevaluation(false);
      return prevMessages;
    });
    ...
    const roomJoinedListener = (data: any) => {
      ...
      if (data.messages && Array.isArray(data.messages)) {
        ...
        const formattedMessages = sortedMessages.map((msg: any) => {
          const { name: sender, id: senderId } = resolveSenderName(
            msg.senderId,
            msg.senderName
          );

          // If user is already loaded, determine correct senderType immediately
          // Otherwise, use temporary placeholder that will be corrected
          let initialSenderType = "other";
          if (user) {
            const isMyMessage = isCurrentUser(senderId, sender);
            initialSenderType = isMyMessage ? "self" : getUserRole(senderId);
          }
          ...
          return {
            ...
            type: msg.type || "text",
            senderType: initialSenderType, // Will be corrected if user wasn't loaded yet
  ...
  const handleNewMessage = useCallback(
    (newMessage: WebSocketChatMessage) => {
      ...
      const formattedMessage: Message = {
        ...
        text: messageContent,
        time: new Date(
          newMessage.timestamp || (newMessage as any).createdAt || new Date()
        ...
        if (data && Array.isArray(data.messages) && data.messages.length > 0) {
          ...
          const formattedMessages = sortedMessages.map((msg: any) => {
            ...
            return {
              ...
              type: msg.type || "text",
              senderType: isCurrentUser(senderId, sender)
                ? "self"
                : getUserRole(senderId),
  ...
  const getRoomName = () => {
    ...
    if (room?.type === "class_group" && room?.classId && classes) {
      const classInfo = classes.find(
        (c: any) => c._id === room.classId || c.id === room.classId
      );
    ...
    if (room?.type === "course_group" && room?.courseId && courses) {
      const courseInfo = courses.find(
        (c: any) => c._id === room.courseId || c.id === room.courseId
      );
  ...
  const getParticipantsText = () => {
    ...
    if (participantCount === 0) {
      ...
      return `${participants.slice(0, 2).join(", ")} and ${
        participantCount - 2
      } others`;
    }
  };
  ...
  const groupMessagesByDate = () => {
    ...
    messages.forEach((message) => {
      ...
      } else if (messageTimestamp && typeof messageTimestamp === "object") {
        messageDate = messageTimestamp as Date;
      } else {
