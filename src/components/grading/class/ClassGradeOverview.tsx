...
const ClassGradeOverview: React.FC<ClassGradeOverviewProps> = ({
  ...
  const [studentGradeOverviews, setStudentGradeOverviews] = useState<
  ...
  const loadClassGradeData = async () => {
    ...
    try {
      // TODO: Implement these API methods when available
      ...
      // For now, use empty arrays until the API methods are implemented
        ...
        (studentsData || []).map(async (student: Student) => {
          try {
            ...
            const classGrades = courseGrades.filter((grade: any) =>
              (coursesData || []).some(
                (course: Course) => course._id === grade.courseId
              )
            );
            ...
            const averagePercentage =
              completedCourses > 0
                ? classGrades.reduce(
                    (sum: number, grade: any) => sum + grade.percentage,
                    0
                  ) / completedCourses
                : 0;
            ...
            console.error(
              `Error loading grades for student ${student._id}:`,
              error
            );
            return {
              student,
  ...
  const filteredStudents = studentGradeOverviews.filter((overview) => {
    const matchesSearch =
      getStudentName(overview.student)
        .toLowerCase()
  ...
  const classStats = {
    ...
    fullyCompleted: studentGradeOverviews.filter(
      (s) => s.completedCourses === s.totalCourses
    ).length,
    partiallyCompleted: studentGradeOverviews.filter(
      (s) => s.completedCourses > 0 && s.completedCourses < s.totalCourses
    ).length,
    notStarted: studentGradeOverviews.filter((s) => s.completedCourses === 0)
      .length,
    classAverage:
      studentGradeOverviews.length > 0
        ? studentGradeOverviews.reduce(
            (sum, s) => sum + s.averagePercentage,
            0
          ) / studentGradeOverviews.length
        : 0,
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Complete view of all student course grades
          </p>
        </div>
            ...
            <CardContent className="pt-0">
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {courses.map((course) => {
                    const courseGrade = overview.courseGrades.find(
                    ...
                    return (
                      ...
                      >
                        ...
                        {courseGrade ? (
                          <div className="flex items-center gap-2">
                            <span className="font-medium">
                              {courseGrade.percentage.toFixed(0)}%
                            </span>
                            <span
