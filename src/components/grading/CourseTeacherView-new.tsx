  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      if (currentTerm) {
        ...
        } else if (currentTerm.data && Array.isArray(currentTerm.data)) {
          termsData = currentTerm.data;
                    ...
                    <Card className="bg-yellow-50 border-yellow-200">
                      <CardContent className="p-4">
                        ...
                        <div className="text-xs text-yellow-700 space-y-1">
                          ...
                          <div>Term ID: {selectedTerm}</div>
                          <div>Students: {students?.length || 0}</div>
                          <div>Assessments: {assessments?.length || 0}</div>
                          <div>Course Grades: {courseGrades?.length || 0}</div>
                        </div>
                      </CardContent>
                    </Card>
                  )}
